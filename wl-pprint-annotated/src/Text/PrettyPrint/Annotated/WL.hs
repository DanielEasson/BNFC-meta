...
class Pretty a where
  pretty     :: Int -> a -> Doc b
  prettyList :: [a] -> Doc b
  prettyList = list . map (pretty 0)

  default pretty :: Show a => Int -> a -> Doc b
  pretty _ = text . show
...
instance Pretty T.Text where
  pretty i = pretty i . T.unpack

instance Pretty TL.Text where
  pretty i = pretty i . TL.unpack
...
instance Pretty a => Pretty (Seq a) where
  pretty _ = prettyList . toList

instance Pretty a => Pretty (NonEmpty a) where
  pretty _ = prettyList . toList

instance (Pretty a, Pretty b) => Pretty (a,b) where
  pretty i (x, y) = tupled [pretty i x, pretty i y]

instance (Pretty a, Pretty b, Pretty c) => Pretty (a,b,c) where
  pretty i (x, y, z) = tupled [pretty i x, pretty i y, pretty i z]

instance Pretty a => Pretty (Maybe a) where
  pretty i = maybe mempty (pretty i)
